...
function renderBusyReplyAutoLibraryPanel(roleId) {
    ...
    const enabled = isBusyReplyEnabled(id);
    const mode = getBusyReplyMode(id);
...
function initChatSettingsUI(roleId) {
    ...
    if (settings.busyReplyMode === 'auto' && settings.busyReplyEnabled !== true) {
        settings.busyReplyEnabled = true;
        saveCurrentChatSettings(id, { busyReplyEnabled: true });
    }
