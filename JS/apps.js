...
function openApp(appId) {
    ...
    if (appId === 'redbook') {
        ...
        if (headerBtn) {
            ...
            headerBtn.style.color = '#111';
            headerBtn.innerHTML = '<i class="fas fa-search" style="font-size: 18px;"></i>';
            headerBtn.onclick = function () {
                if (window.RedbookForumApp && typeof window.RedbookForumApp.openGeneratorDrawer === 'function') {
                    window.RedbookForumApp.openGeneratorDrawer();
                }
            };
        }
...
window.deleteRole = function (id) {
    ...
    if (window.chatData && window.chatData[id]) {
        ...
        try { localStorage.setItem('wechat_chatData', JSON.stringify(window.chatData)); } catch (e) {}
    } else {
        try {
            const stored = JSON.parse(localStorage.getItem('wechat_chatData') || '{}');
            if (stored && stored[id]) {
                delete stored[id];
                localStorage.setItem('wechat_chatData', JSON.stringify(stored));
            }
        } catch (e) {}
    }

    if (window.chatUnread && window.chatUnread[id] != null) {
        delete window.chatUnread[id];
        try { localStorage.setItem('wechat_unread', JSON.stringify(window.chatUnread)); } catch (e) {}
    } else {
        try {
            const storedUnread = JSON.parse(localStorage.getItem('wechat_unread') || '{}');
            if (storedUnread && storedUnread[id] != null) {
                delete storedUnread[id];
                localStorage.setItem('wechat_unread', JSON.stringify(storedUnread));
            }
        } catch (e) {}
    }
