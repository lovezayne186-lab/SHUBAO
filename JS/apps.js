...
// 1. 定义所有 App 的核心数据
window.appList = [
    { name: "秘密中心", id: "secret", iconClass: "fas fa-user-secret" },
        ...
        <div class="w-full h-full bg-gray-50 flex flex-col relative overflow-hidden font-sans">
            ...
            <div class="w-full h-full pb-16 relative">
                ...
                <div id="tab-chat" class="chat-tab-page w-full h-full flex flex-col relative">
                    ...
                    <div id="wechat-import-chooser" class="fixed inset-0 z-50 hidden">
                        <div class="absolute inset-0 bg-black/40" onclick="closeWechatImportChooser()"></div>
                        <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-3xl bg-white p-4 shadow-2xl" style="width:min(90vw, 320px);">
                            <div class="text-base font-bold text-gray-800">选择导入类型</div>
                            <div class="mt-1 text-sm text-gray-500">请先选择要导入的是角色备份，还是普通角色卡 JSON。</div>
                            <div class="mt-4 flex flex-col gap-3">
                                <button type="button" class="rounded-2xl bg-pink-500 px-4 py-3 text-left text-white font-semibold active:scale-[0.99]" onclick="startWechatImport('backup')">
                                    导入角色备份
                                    <div class="mt-1 text-xs text-pink-100 font-normal">导入之前导出的单独角色完整备份，包含聊天记录和相关数据。</div>
                                </button>
                                <button type="button" class="rounded-2xl bg-gray-100 px-4 py-3 text-left text-gray-800 font-semibold active:scale-[0.99]" onclick="startWechatImport('role')">
                                    导入角色
                                    <div class="mt-1 text-xs text-gray-500 font-normal">导入普通 JSON 角色卡，只创建/更新角色基础资料。</div>
                                </button>
                            </div>
                            <button type="button" class="mt-3 w-full rounded-2xl px-4 py-3 text-sm text-gray-500 hover:bg-gray-50" onclick="closeWechatImportChooser()">取消</button>
                        </div>
                    </div>
...
window.importWechatContactFromInput = async function (fileInput) {
    ...
    const normalizeRoleProfile = function (rawProfile, fallbackName) {
        ...
        if (typeof profile.remark === 'string') profile.remark = profile.remark.trim();
        if (typeof profile.desc === 'string') profile.desc = profile.desc.trim();
        if (typeof profile.style === 'string') profile.style = profile.style.trim();
        if (typeof profile.schedule === 'string') profile.schedule = profile.schedule.trim();
        if (typeof profile.avatar === 'string') profile.avatar = profile.avatar.trim();
        if (typeof profile.gender === 'string') profile.gender = profile.gender.trim();
        if (typeof profile.worldbookId === 'string') {
            profile.worldbookId = profile.worldbookId.trim();
        }
        if (Array.isArray(profile.worldbookId)) {
            profile.worldbookId = profile.worldbookId.slice();
        }
        return profile;
    };
    ...
    const importRoleCardFromJson = async function (json) {
        ...
        const sourceData = isObject(json.data) ? json.data : null;
        const sourceProfile = isObject(json.profile)
            ? json.profile
            : (isObject(json.charProfile) ? json.charProfile : null);
        const source = Object.assign({}, sourceData || {}, sourceTop || {}, sourceProfile || {});
        ...
        const hasCharaCardShape = cardSpec === 'chara_card_v3'
            ...
            || (typeof sourceData?.description === 'string' && /<character>|<writing_rule>|<info>/i.test(sourceData.description));
        ...
        loadWechatChatList(true);
        alert('✅ 已导入角色' + (targetId !== candidateId ? '（ID 冲突已自动改名）' : ''));
    };
