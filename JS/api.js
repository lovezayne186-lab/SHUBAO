...
function extractScheduleTimeTokens(line) {
    ...
    const source = '([0-2]?\\d(?::[0-5]?\\d)?|[零〇一二两三四五六七八九十]{1,3}(?:点(?:半|一刻|三刻|\\d{1,2}分?)?)?)';
...
function extractScheduleTimeRange(line) {
    const text = String(line || '');
    const source = '([0-2]?\\d(?::[0-5]?\\d)?|[零〇一二两三四五六七八九十]{1,3}(?:点(?:半|一刻|三刻|\\d{1,2}分?)?)?)\\s*[-~—–至到]\\s*([0-2]?\\d(?::[0-5]?\\d)?|[零〇一二两三四五六七八九十]{1,3}(?:点(?:半|一刻|三刻|\\d{1,2}分?)?)?)';
...
function parseSchedulePlanFromText(rawText) {
    ...
    const directSleep = normalized.match(/([零〇一二两三四五六七八九十\d]{1,3}(?:点|时)?(?:半|一刻|三刻|\d{1,2}分?)?)\s*(?:起床|起|醒).*?([零〇一二两三四五六七八九十\d]{1,3}(?:点|时)?(?:半|一刻|三刻|\d{1,2}分?)?)\s*(?:睡|睡觉|休息)/);
    if (!plan.sleepStart && !plan.sleepEnd && directSleep) {
        setSleepByTokens(directSleep[2], directSleep[1], 0.7);
    }

    const reverseSleep = normalized.match(/([零〇一二两三四五六七八九十\d]{1,3}(?:点|时)?(?:半|一刻|三刻|\d{1,2}分?)?)\s*(?:睡|睡觉|休息).*?([零〇一二两三四五六七八九十\d]{1,3}(?:点|时)?(?:半|一刻|三刻|\d{1,2}分?)?)\s*(?:起床|起|醒)/);
...
function writeBusyWakeOverride(roleId, payload) {
    ...
    const expiresAt = Number(src.expiresAt) || (createdAt + 2 * 60 * 1000);
    const data = {
        createdAt: createdAt,
        expiresAt: expiresAt,
...
function noteBusyUserInterruption(roleId, userText) {
    ...
    if (decision.shouldWake) {
        writeBusyWakeOverride(rid, {
            createdAt: now,
            expiresAt: now + (decision.urgent ? 4 * 60 * 1000 : 2 * 60 * 1000),
...
function buildBusyShortcutPayload(roleId, busyState, userMessage) {
    ...
    if (mode === 'auto') {
        ...
        const reply = fillBusyReplyTemplate(choice, {
            reason: reason,
        ...
        return {
            ...
            reply: reply.length ? reply : ['我现在有点忙。', '晚点再回你。'],
            system_event: null,
...
async function callAI(systemPrompt, historyMessages, userMessage, onSuccess, onError, options) {
    ...
    const busyWakeOverride = modernPromptMeta && modernPromptMeta.sceneCode === 'wechat_private_v2'
        ? consumeBusyWakeOverride(roleId)
        : null;
