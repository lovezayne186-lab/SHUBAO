...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (floatWindow && overlay) {
        ...
        const onFloatDragStart = function (e) {
            const state = window.voiceCallState || {};
            if (!state.active) return;
        ...
        const onFloatDragMove = function (e) {
            ...
            floatWindow.style.left = String(dragOriginLeft + dx) + 'px';
            floatWindow.style.top = String(dragOriginTop + dy) + 'px';
            floatWindow.style.right = 'auto';
            floatWindow.style.bottom = 'auto';
            e.preventDefault();
        };

        const onFloatDragEnd = function () {
            if (!isDraggingFloat) return;
            setTimeout(function () {
                floatMoved = false;
            }, 0);
        ...
        floatWindow.addEventListener('mousedown', onFloatDragStart);
        floatWindow.addEventListener('touchstart', onFloatDragStart, { passive: false });
