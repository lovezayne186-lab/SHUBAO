...
function getAiMessageNotificationPreview(msg) {
    ...
    if (type === 'text') {
        const parsed = parseTranslatedBubbleText(msg.content);
        if (parsed) {
...
function parseTranslatedBubbleText(rawText) {
    ...
    if (quoteOpen !== -1 && quoteClose > quoteOpen) {
        ...
        if (parenOpen !== -1 && parenClose > parenOpen) {
            foreignText = stripMarkdownDisplayArtifacts(body.slice(0, parenOpen));
            translationText = stripMarkdownDisplayArtifacts(body.slice(parenOpen + 1, parenClose));
        }
    }
    ...
    function pushText(text) {
        ...
        for (let i = 0; i < coarseParts.length; i++) {
            ...
            for (let j = 0; j < lineParts.length; j++) {
                const item = String(lineParts[j] || '').trim();
                if (!item) continue;
...
function invokeAIWithCommonHandlers(systemPrompt, cleanHistory, userMessage, roleId, headerTitle, oldTitle) {
    ...
    if (typeof window.callAI === 'function') {
        ...
        const tryAttachQuote = function (msgObj) {
                    ...
                    if (pendingInlineQuoteText && !(msgObj.quote && msgObj.quote.text)) {
                        if (!msgObj.quoteId) msgObj.quoteId = pendingQuoteId || 'user_last';
                        if (msgObj.quoteId) resolveQuoteBlockForMessage(roleId, msgObj);
                        if (!(msgObj.quote && msgObj.quote.text)) {
                            msgObj.quote = {
                                name: '引用',
...
function appendMessageToDOM(msg) {
    ...
    if (msg.type === 'listen_invite') {
        ...
        if (stickerMatch) {
            ...
            const translatedPayload = msg.role === 'ai' ? parseTranslatedBubbleText(displayContent) : null;
...
function bindMessageLongPressEdit(row, msg) {
    ...
    function handleEdit() {
        ...
        const textEl = row.querySelector('.msg-text') || row.querySelector('[data-translation-bubble="1"]');
        if (textEl) {
            const rendered = String(newText).replace(/\n/g, '<br>');
            textEl.innerHTML = rendered;
            if (textEl.hasAttribute && textEl.hasAttribute('data-translation-bubble')) {
