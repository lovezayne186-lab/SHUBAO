(function () {
  ...
  function updateRoomForGroup(roleId) {
    ...
    if (api.isGroupChatRole(roleId)) {
      ...
      const members = api.getGroupMembers(roleId);
  ...
  function ensureSenderLabel(row, name) {
    if (!row || !name) return;
    ...
    const existing = bubble.querySelector('.group-sender-label');
    if (existing) {
  ...
  function wrapSendMessage() {
    ...
    window.sendMessage = async function (...args) {
      const roleId = window.currentChatRole;
      const beforeLength = Array.isArray(window.chatData?.[roleId]) ? window.chatData[roleId].length : 0;
      ...
      const history = Array.isArray(window.chatData?.[roleId]) ? window.chatData[roleId] : [];
      const lastMsg = history[history.length - 1];
      const sentNewUserMessage = history.length > beforeLength && lastMsg?.role === 'me';
      if (!sentNewUserMessage) {
        return result;
      }

      if (autoReplyPendingRoleId === roleId) {
        return result;
      }

      autoReplyPendingRoleId = roleId;
      try {
        await getGroupApi().triggerGroupAI(roleId);
      } finally {
        autoReplyPendingRoleId = '';
      }
      return result;
    };
  }
  ...
  function wrapResolveMessageSenderName() {
    ...
    window.resolveMessageSenderName = function (roleId, msg, ...rest) {
      if (isGroupRole(roleId) && msg?.role === 'ai') {
  ...
  function wrapCreateMessageRow() {
    ...
    window.createMessageRow = function (msg, ...rest) {
      ...
      ensureSenderLabel(row, member?.name || msg?.senderName || '群成员');
      return row;
    };
  }
