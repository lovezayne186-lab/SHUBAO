(function () {
  ...
  function findGroupMessageByTimestamp(roleId, targetTimestamp) {
    const token = String(targetTimestamp || '').trim();
  ...
  function normalizeGroupAction(roleId, item, member) {
    ...
    if (type === 'system_message') {
      const text = readActionText(item, ['content', 'message', 'text']);
      if (!text) return null;
      return { messages: [buildSystemEventMessage(text, timestamp)], effects: [] };
    }

    if (type === 'send_and_recall') {
      return {
        messages: [buildSystemEventMessage(`${actorName}撤回了一条消息`, timestamp)],
        effects: []
      };
    }
    ...
    if (type === 'change_group_name') {
      ...
      return {
        messages: [buildSystemEventMessage(`${actorName}修改群名为“${newName}”`, timestamp)],
        effects: [{ type: 'change_group_name', newName }]
      };
    }
    ...
    if (type === 'change_group_avatar') {
      ...
      return {
        messages: [buildSystemEventMessage(`${actorName}修改了群头像：${avatarName}`, timestamp)],
        effects: [{ type: 'change_group_avatar', avatarName }]
      };
    }
    ...
    if (type === 'send_private_message') {
      ...
      return {
        messages: [buildSystemEventMessage(`${actorName}悄悄给你发来了一条私信：${privateText}`, timestamp)],
        effects: []
      };
    }
  ...
  async function triggerGroupAI(roleId) {
    ...
    return new Promise((resolve, reject) => {
      window.callAI(
        ...
        async (rawText) => {
          try {
            const parsedPayload = parseGroupAiResponse(rawText, roleId);
