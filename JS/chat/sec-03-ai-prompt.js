...
function buildPromptContextBundle(roleId, options) {
    ...
    const personalizedOn = settings.personalizedResponseEnabled !== false;
    const summaryLinesFromSettings = typeof settings.summaryLines === 'number' ? settings.summaryLines : 0;
    const maxSummaryLines = opts.maxSummaryLines != null
        ? opts.maxSummaryLines
        : (summaryLinesFromSettings > 0 ? summaryLinesFromSettings : 12);
    return {
        roleId: rid,
        profile: profile,
        ...
        worldBookPromptText: typeof buildWorldBookPrompt === 'function'
            ...
            : '',
        memoryArchivePrompt: personalizedOn ? buildMemoryArchivePromptForScene(rid, { historyForApi: historyForApi }) : '',
        recentSystemEventsPrompt: opts.includeRecentSystemEvents === false || !personalizedOn
            ? ''
            : buildRecentSystemEventsPrompt(historyForApi),
        continuityPrompt: opts.includeContinuity === false || !personalizedOn
            ? ''
            : buildContinuityPrompt(historyForApi, String(profile.nickName || profile.name || rid || 'TA').trim() || 'TA'),
        recentChatSummary: opts.includeRecentChatSummary === false || !personalizedOn
            ? ''
            : buildRecentChatSummaryTextForPrompt(historyForApi, maxSummaryLines || 12),
        giftTaskPrompt: personalizedOn ? buildGiftTodoPromptForScene(rid) : '',
