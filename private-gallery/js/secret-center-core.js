(function (global) {
    ...
    function writeSceneCache(scene, roleId, payload) {
        ...
        const ok = writeLocalStorage(key, JSON.stringify(body));
        if (ok && /^shadow_/.test(asString(scene))) {
            markSceneGenerated(roleId, scene, asString(body.source || body.batchSource || 'cache_write'));
        }
        return ok;
    }
